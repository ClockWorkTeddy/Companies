...
namespace Simplified
{
    public abstract partial class ViewModelBase
    {
        ...
        private static ConditionalWeakTable<RelayCommand, EventHandler> requerySuggesteds = new();
        ...
        private RelayCommand GetCommand<T>(Delegate execute, Delegate? canExecute, Delegate? converter, string commandName, bool isVoid)
        {
            if (!commands.TryGetValue(commandName, out RelayCommand? command))
            {
                ...
                EventHandler requerySuggested = (s, e) => command.RaiseCanExecuteChanged();

                requerySuggesteds.Add(command, requerySuggested);
        ...
        protected static readonly ArgumentException canExecuteException = new ArgumentException("null не разрешён", "canExecute");
        protected static readonly ArgumentException converterException = new ArgumentException("null не разрешён", "converter");
        protected static readonly ArgumentException commandNameException = new ArgumentException("null, Empty и только пробелы не разрешёны", "commandName");
    }
}
